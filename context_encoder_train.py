...
import context_encoder_utils

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', default='imagenet', help='cifar10 | lsun | imagenet | folder | lfw ')
parser.add_argument('--dataroot', default='../dataset/EUS_bulk/train', help='path to dataset')
...
for epoch in range(resume_epoch, opt.niter):
    ...
    torch.save({'epoch': epoch + 1,
               ...
               '../new_save/model/netG_streetview.pth')
    torch.save({'epoch': epoch + 1,
                'state_dict': netD.state_dict()},
               '../new_save/model/netlocalD.pth')