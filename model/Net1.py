...
class Net(nn.Module):
    def __init__(self, num_classes, pre_trained=False):
        ...
        self.relationNet = RelationNetwork(cfg=[64, 128, 256, 512, 512])
...
class RelationNetwork(nn.Module):
    def __init__(self, cfg):
        ...
        self.att_bulk = nn.ModuleList([RelationalAttention(cfg[i]).cuda() for i in range(len(cfg))])
...
if __name__ == '__main__':
    net = Net(num_classes=5).cuda(1)
    print(net)
    # net = vgg16_bn().cuda()
    x = torch.ones((32, 3, 224, 224)).cuda(1)
    c_pred = net(x)
    print(c_pred)
    # print(pred)
    # print(net.features[:6](x).shape[1],
    #       net.features[:13](x).shape[1],
    #       net.features[:23](x).shape[1],
    #       net.features[:33](x).shape[1],
    #       net.features[:43](x).shape[1]
    #       )
